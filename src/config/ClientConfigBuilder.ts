...
export class ClientConfigBuilder extends ConfigBuilder {
  ...
  protected getOutput(): Output {
    ...
    const base = super.getOutput();
    const absoluteSrcDir = path.join(this.ctx.cwd, srcDir);
    ...
    return {
      ...
      chunkFilename: this.isDev
        ? "static/js/[name].chunk.js"
        : "static/js/[name].[chunkhash:8].chunk.js",

      // Point sourcemap entries to original disk location (format as URL on Windows)
      devtoolModuleFilenameTemplate: this.isDev
        ? info => path.resolve(info.absoluteResourcePath).replace(/\\/g, "/")
        : info =>
            path
              .relative(absoluteSrcDir, info.absoluteResourcePath)
              .replace(/\\/g, "/"),
    };
  }
  ...
  protected getManifestPlugin(): Plugin | undefined {
    ...
    return new ManifestPlugin({
      ...
      fileName: ConfigBuilder.ASSET_MANIFEST_FILE_NAME,
    });
  }

  protected getHotModuleReplacementPlugin(): HotModuleReplacementPlugin {
  ...
  protected getDevServer(): undefined | WebpackDevServer.Configuration {
    ...
    const { publicPath } = this.getOutput();
    const { appProtocol, appFullDevHost } = this.ctx;
  ...
  protected getDevTool(): Options.Devtool {
    return this.isDev ? "eval" : "source-map";
  }

  protected getTarget(): "web" {
    return "web";
  }
